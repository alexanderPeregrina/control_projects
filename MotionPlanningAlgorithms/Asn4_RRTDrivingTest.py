...
for i in range(8):
    ...
    for k in range (rrt.iterations):
        ...
        psi = atan2(goal[1]-new[1], goal[0]-new[0]) + random.uniform(-0.2, 0.2) #add some noise to the heading
        ...
        if not rrt.isInObstacle(rrt.nearestNode, new):
            ...
            xTP, yTP, pathDistance = returnDubinsPath(rrt.nearestNode.locationX, rrt.nearestNode.locationY, rrt.nearestNode.heading, new[0], new[1], psi, rrt.turnRadius)
            if pathDistance is not None and not rrt.isDubinsInObstacle(xTP, yTP): #update
            
                #TODO: add new as a child with other attributes
                rrt.addChild(new[0], new[1], psi, pathDistance, xTP, yTP)
                plt.pause(0.05)
                plt.plot(xTP, yTP,'k')
                ...
                xTP, yTP, pathDistance = returnDubinsPath(new[0], new[1], psi, rrt.goal.locationX, rrt.goal.locationY, rrt.goal.heading, rrt.turnRadius)
                ...
                if pathDistance is not None and not rrt.isDubinsInObstacle(xTP, yTP): #update       
                    ...
                    if projectedCost < rrt.goalCosts[-1]: #update this
                        #set the nearestNode to the newNode (again.. how can you find the newNode (treeNode) object)
                        rrt.nearestNode = rrt.nearestNode.children[-1] #update this
                        #add goal as the child with xTP, yTP, pathDistance
                        rrt.addChild(rrt.goal.locationX, rrt.goal.locationY, rrt.goal.heading, pathDistance, xTP, yTP)
                        #retrace path
                        rrt.retracePath()
