...
class RRTAlgorithm():
    ...
    def steerToPoint(self, locationStart, locationEnd):
        ...
        point[0] = min(self.grid.shape[1]-1, max(0, point[0]))
        point[1] = min(self.grid.shape[0]-1, max(0, point[1]))
    ...
    def isDubinsInObstacle(self, xTrajectory, yTrajectory):
        ...
        for i in range(len(xTrajectory)):
            x = round(xTrajectory[i])
            y = round(yTrajectory[i])
            if self.grid[min(self.grid.shape[0]-1, y), min(self.grid.shape[1]-1, x)] == 1:
                return True
        return False    
    ...
    def retraceRRTPath(self,goal):
        ...
        for i in range(len(goal.xTP)-1, -1, -1):
