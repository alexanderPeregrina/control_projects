...
for i in range(rrtStar.iterations):
    ...
    heading = atan2(goal[1] - new[1], goal[0] - new[0]) + random.uniform(-0.5, 0.5)
    ...
    if not rrtStar.isInObstacle(rrtStar.nearestNode, new):
        ...
        if pathDistance is not None and not rrtStar.isDubinsInObstacle(xTP, yTP): #update this
        
            #TODO: find neighbouring nodes
            rrtStar.findNeighbouringNodes(rrtStar.nearestNode, new)
            ...
            min_cost = rrtStar.findPathDistance(min_cost_node) + pathDistance
