...
class RRTStarAlgorithm():
    ...
    def steerToPoint(self, locationStart, locationEnd):
        ...
        point = np.array([min(self.grid.shape[1]-1, max(point[0], 0)), min(self.grid.shape[0]-1, max(point[1], 0))])
    ...
    def isDubinsInObstacle(self, xTrajectory, yTrajectory):
        ...
        for i in range(len(xTrajectory)):
            x = min(self.grid.shape[1]-1, round(xTrajectory[i]))
            y = min(self.grid.shape[0]-1, round(yTrajectory[i]))
            if self.grid[y,x] == 1:
                return True
